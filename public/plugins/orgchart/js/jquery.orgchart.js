...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    hideChildren: function ($node) {
      ...
      var childrenTableWidth = $($node.closest('table')).outerWidth(true);
      var currentTransform = $chart.css('transform').replace('matrix(1, 0, 0, 1, ', '').replace(')', '').split(', ');
      var currentTransformX = parseInt(currentTransform[0]);
      var currentTransformY = parseInt(currentTransform[1]);
      ...
      // customization: change chart position to center
      var $topNode = $($chart.find('.node').get(0));
      var diffLeft = $topNode.offset().left - $node.offset().left;
      var newX = window.parseInt(($chart.outerWidth(true)/2) - ($node.offset().left - $chart.offset().left) - ($node.outerWidth(true)/2));

      if((typeof $node.attr('data-parent') == 'undefined')) // root parent node
      {
        $chart.css('transform', 'matrix(1, 0, 0, 1, ' + newX + ', ' + 0 + ')');
      }
      else if(childrenTableWidth > $chartcontainer.width())
      {
        if(diffLeft < 0) // right side nodes
        {
          newX = window.parseInt(($chart.outerWidth(true)/2) - ($node.offset().left - window.parseInt(diffLeft / 1.25) - $chart.offset().left) - ($node.outerWidth(true)/2));
        }
        else // left side nodes
        {
          newX = window.parseInt(($chart.outerWidth(true)/2) - ($node.offset().left + window.parseInt(diffLeft * 1.25) - $chart.offset().left) - ($node.outerWidth(true)/2));
        }

        $chart.css('transform', 'matrix(1, 0, 0, 1, ' + newX + ', ' + currentTransformY + ')');
      }
    },
    //
