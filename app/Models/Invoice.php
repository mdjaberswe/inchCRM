...
class Invoice extends BaseModel
{
	...
	protected $fillable = ['account_id', 'contact_id', 'deal_id', 'project_id', 'sale_agent', 'number', 'reference', 'subject', 'status', 'invoice_date', 'date_pay_before', 'currency_id', 'discount_type', 'sub_total', 'total_tax', 'total_discount', 'adjustment', 'grand_total', 'payment', 'recurring', 'term_condition', 'note'];
	...
	public static function validate($data)
	{
		...
		$invoice_date_minus = date('Y-m-d', strtotime($invoice_date . ' -1 day'));		

		$unique_number = "unique:invoices,number";
		if(isset($data['id'])) :
			$id = $data['id'];
			$unique_number = "unique:invoices,number,$id";
		endif;

		$status = "required|in:draft,unpaid";		
		if(isset($data['current_status'])) :
			$current_status = $data['current_status'];
			if($current_status == 'paid' || $current_status == 'partially_paid') :
		...
		$rules = ["account_id"		=> "required|exists:accounts,id,deleted_at,NULL",
				  ...
				  "date_pay_before"	=> "date|after:$invoice_date_minus",
	...
	*/
	...
	{
		return 'INV-' . $this->number_format;
	}

	public function getNumberHtmlAttribute($link_css = null, $hide_tooltip = null)
	{
		$tooltip = '';
		if(isset($this->subject) && $this->subject != '' && $hide_tooltip == null) :
			$tooltip = " data-toggle='tooltip' data-placement='top' title='" . str_limit($this->subject, 25, '.') . "'";
		endif;

		$link_style = is_null($link_css) ? '' : $link_css;
			
		$number = "<a class='" . $link_style . "' href='" . route('admin.sale-invoice.show', $this->id) . "'$tooltip>" . $this->name . "</a>";
		return $number;
	}
	...
	public function getDateHtmlAttribute()
	{
		...
		if(isset($this->date_pay_before)) :
			$span_class = 'shadow';
			$date .= "<span class='c-danger' data-toggle='tooltip' data-placement='right' title='Date Pay Before'>" . $this->date_pay_before . "</span>";
			$date .= '<br>';
		endif;

		if(isset($this->invoice_date)) :
			$date .= "<span class='" . $span_class . "' data-toggle='tooltip' data-placement='right' title='Invoice Date'>" . $this->invoice_date . "</span>";
		endif;
	...
	public function getStatusHtmlAttribute()
	{
		...
		switch($status) :
			...
			case 'paid' :
				$status_html = "<span class='btn btn-success status'>Paid</span>";
			break;

			case 'partially_paid' :
				$status_html = "<span class='btn btn-warning status'>Partially Paid</span>";
			break;

			case 'unpaid' :
				$status_html = "<span class='btn btn-danger status'>Unpaid</span>";
			break;
	...
	*/
	...
	{
		return $this->hasOne(Expense::class, 'converted_invoice_id');
	}
