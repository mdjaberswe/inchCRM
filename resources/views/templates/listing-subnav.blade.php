...
@section('content')
	...
	<div class='row'>
	    ...
	    <div class='col-xs-12 col-sm-9 col-md-9 col-lg-10 div-type-a'>
	        <h4 class='title-type-a bold'>@if(isset($page['list_title'])) {!! $page['list_title'] !!} @else {!! $page['item'] !!} List @endif</h4>
	        ...
	        <table id='datatable' class='table display responsive {!! $table['class'] or null !!}' cellspacing='0' width='100%'>
	        	<thead>
	        		<tr>
		        		...
		        		@foreach($table['thead'] as $key => $thead)
		        			@if(is_array($thead))
		        				<th data-priority='{!! ($key+3) !!}' data-orderable='{!! $thead['orderable'] or 'true' !!}' data-class-name='{!! $thead['data_class'] or null !!}' class='{!! $thead['class'] or null !!}' style='{!! $thead['style'] or null !!}'>{!! $thead[0] !!}</th>
		        			@else
...
@push('scripts')
	<script>
		$(document).ready(function()
		{
			...
			var datatableDom = "<<'bulk'>lBfr<'table-responsive't>ip>";
			var tableColumns = [ {!! $table['json_columns'] !!} ];

			@if(isset($table['custom_filter']) && $table['custom_filter'] !== false)
				datatableDom = "<<'bulk'>lBf<'custom-filter'>r<'table-responsive't>ip>";
			@endif
